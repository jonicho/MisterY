...
public class PositionPredicter {
	...
	/** Predicts all Possible Movements the Specified player can make from the specified station with his tickets
	 * @param station the station the player is on
	...
	private ArrayList<Station> PredictPositions(Station station, Player player) {
		...
		if (player.getBusTickets() > 0) hasBusTickets = true;
		else hasBusTickets = false;
		if (player.getTaxiTickets() > 0) hasTaxiTickets = true;
		else hasTaxiTickets = false;
		if (player.getUndergroundTickets() > 0) hasTrainTickets = true;
		else hasTrainTickets = false;
		ArrayList<Link> stationLinks = station.getLinks();
		ArrayList<Link> possibleLinks = new ArrayList<Link>();
		for (Link l : stationLinks) {
			if (l.isBus() && hasBusTickets) possibleLinks.add(l);
			if (l.isUnderground() && hasTrainTickets) possibleLinks.add(l);
			if (!l.isBus() && !l.isUnderground() && hasTaxiTickets) possibleLinks.add(l);
		}
	...
	/** Returns if for the given parameters the can be a prediction with 100% accuracy
	 * @param station 
	 * @param player 
	 * @return
	 */
